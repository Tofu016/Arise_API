...
class IndoorUploads_API extends MY_Controller
{
    ...
    public function serve()
    {
        ...
        // &format=jpeg: the mobile app's 3D viewers are only handed JPEG,
        // at a size a phone can handle, so it asks for a downscaled JPEG
        // copy (see Photo_preview), optionally &width= one of
        // Photo_preview::WIDTHS. Web never sends it and keeps getting the
        // original, untouched. If this server can't convert the file, the
        // original is still better than nothing.
        if ($this->input->get('format') === 'jpeg') {
            $previews = $this->photoPreview(); // also loads the Photo_preview class
            $width = Photo_preview::allowedWidth($this->input->get('width'));
            if ($width === null) {
                http_response_code(400);
                echo 'Invalid width.';
                return;
            }
            $preview = $previews->jpegFor($fullPath, $width);
            if ($preview['ok']) {
                $fullPath = $preview['full_path'];
                $contentType = 'image/jpeg';
            }
        }

        // Connection: close — under investigation: on the phone, every
        // photo request after the first stalled ~30 s inside fetch(),
        // the signature of a stale pooled keep-alive connection. Remove
        // once the Apache keep-alive settings are confirmed as the cause.
